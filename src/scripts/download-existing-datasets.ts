/**
 * Download and convert existing datasets to JoSan annotation format.
 *
 * Usage:
 *   npx tsx src/scripts/download-existing-datasets.ts
 *
 * Prerequisites:
...
const DATA_DIR = "./data/existing-datasets";

/**
 * Create Python script to download datasets from HuggingFace
 */
function createDownloadScript(): string {
  return `
import os
import json
...
async function main(): Promise<void> {
  console.log("🚀 JoSan Dataset Downloader\n");
  console.log("This script downloads existing Filipino hate speech datasets");
  console.log("and converts them to JoSan annotation format.\n");
  ...
  const pythonScript = createDownloadScript();
  const scriptPath = `${DATA_DIR}/download_datasets.py`;
  writeFileSync(scriptPath, pythonScript);
  console.log(`📝 Created Python script: ${scriptPath}\n`);

  // Run Python script
  console.log("🐍 Running Python download script...\n");
  try {
    execSync(`python "${scriptPath}"`, { stdio: "inherit" });
  } catch {
    console.log("\n⚠️  Python script execution failed.");
    console.log("   Try running manually:");
    console.log(`   python "${scriptPath}"`);
  }

  console.log("\n📋 Next Steps:");
  console.log("1. Open http://localhost:5173/annotation.html");
  console.log(
    `2. Click "Import JSON/CSV" and select ${DATA_DIR}/combined_annotations.json`,
  );
  console.log("3. Review and verify imported annotations");
  console.log(
    '4. Use "Export 80/20 Split" to create training/validation files',
  );
}
